...
# The default symfony directory
PROJECT_DIRECTORY=$(shell pwd)/symfony
...
ifeq ("$(wildcard $(PROJECT_DIRECTORY))","")
	- docker-compose -f docker/docker-compose.dev.yml exec php-cli composer --no-scripts create-project symfony/framework-standard-edition symfony

	# Install using no scripts, run the script after the installation has finished and the parameters is in place
	- cp config/symfony/parameters.yml symfony/app/config/parameters.yml
	
	# Run the post install scripts
	- docker-compose -f docker/docker-compose.dev.yml exec php-cli composer -d "symfony" --ansi run-script post-install-cmd
endif

# Update the project and the dependencies
update-project:
	- docker-compose -f docker/docker-compose.dev.yml exec php-cli composer -d "symfony" --ansi update
